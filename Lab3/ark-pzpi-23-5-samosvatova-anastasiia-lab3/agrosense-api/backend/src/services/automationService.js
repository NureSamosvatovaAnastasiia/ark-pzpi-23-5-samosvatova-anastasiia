...
class AutomationService {
  ...
  async calculateTrend(sensorType, greenhouseId) {
  ...
  async handleAirHumidity(greenhouse, crop, currentVal, trend) {
      ...
      const tempSensor = await this.findSensor(greenhouse.id, 'temperature');
      const lastTempRead = await this.getLastReading(tempSensor?.id);
      const currentTemp = lastTempRead ? parseFloat(lastTempRead.value) : 20; 
      ...
      if (predictedVal < min) {
          if (humidifier) {
              ...
              const maxWaterGM3 = this.calculateMaxWaterInAir(currentTemp);

              const deficitPercent = target - currentVal; 
              const deficitGM3 = maxWaterGM3 * (deficitPercent / 100);

              const volume = parseFloat(greenhouse.areaSqMeters) * parseFloat(greenhouse.heightMeters || 3);
              const totalWaterNeededMl = deficitGM3 * volume; 

              const capacityMlH = parseFloat(humidifier.capacity) || 500; 
              
              let power = (totalWaterNeededMl / capacityMlH) * 100 * 2; 
          
              power -= (trend * 1000); 

              power = Math.min(100, Math.max(10, power));

              await this.setActuator(humidifier, true, power, `AUTO:Humidify(Need ${totalWaterNeededMl.toFixed(0)}ml)`);
          }
      } 
